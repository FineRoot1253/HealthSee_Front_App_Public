...
class PlanMenuBar extends StatelessWidget {

  var controller;
  final String title;
  final String content;

  PlanMenuBar(this.controller, this.title, [this.content]);
  ...
  callback(context) {
    if (title.endsWith("선택")) {
      print("여기");
      return () => PlanWriteController.to.scaffoldKey.currentState.openDrawer();
    }
    if (title.endsWith("설명")) {
      return () async {
        await Get.toNamed("/planCopy", arguments: controller);
      };
    }
  }
}
