...
class AlbumMenuBar extends StatelessWidget {
  final String title;
  final String content;
  final AlbumMainController controller;

  AlbumMenuBar(this.controller, this.title, this.content);
                  ...
                  children: <Widget>[
                    ...
                    ConditionalBuilder(
                      fallback: (context) => Container(
                        height: 30,),
                      condition: (controller.albumListController.currentNickname ==
                          AccountController.to.nickname || title.endsWith("년") ),
  ...
  callback(context) {
    if (title.endsWith("년")){
      return () => controller.selectYear(context, controller.albumListController.currentNickname);
    }
    if (title.endsWith("월")) {
      return () async {
        await Get.toNamed("/albumWrite", arguments: controller);
      };
